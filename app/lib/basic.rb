  ...
  #    return path.relative_path_from(project_dir)
    ...
    def load_annot run, meta, relative_filepath, h_data_types
      ...
      if !meta['type'] or !meta['dataset_size']
        ...
        cmd = "java -jar #{APP_CONFIG[:local_asap_run_dir]}/ASAP.jar -ultralight -T ExtractMetadata -f #{loom_path} #{type_txt} -meta #{meta['name']}"
        puts cmd
        res_json =`#{cmd}`
        puts res_json
        meta_compl = {}
...
#        end
      ...
      if predictable
        ...
        h_var['nber_rows'] = h_output_json['nber_cols']
      end
...
#      project.broadcast step.id
      ...
      if h_output_json['metadata']
        h_output_json['metadata'].each do |metadata|
      ...
      h_output_files.each_key do |k|
        h_output_files[k].each_key do |k2|           
          ...
          if h_output_files[k][k2]["types"].include?("dataset")
            ...
            if dataset_name
              ...
              if dataset_name == '/matrix' or dataset_name.match(/^\/layers\//)
                h_output_files[k][k2]["types"].push((h_output_json and h_output_json['is_count_table'].to_i == 1) ? "int_matrix" : "num_matrix")
                h_output_files[k][k2]["dataset_size"] = (h_output_json['nber_rows'] and h_output_json['nber_cols']) ? 4 * h_output_json['nber_rows'] * h_output_json['nber_cols'] : nil
                
                h_data = {
                  'nber_cols' => h_output_json['nber_cols'],
                  'nber_rows' =>  h_output_json['nber_rows'],
                  'type' => 'NUMERIC',
                  'on' => 'EXPRESSION_MATRIX',
                  'dataset_size' => h_output_files[k][k2]["dataset_size"],                    
                  'name' => dataset_name,
                  'count' => (h_output_json and h_output_json['is_count_table'].to_i == 1) ? true : false
                }
                puts "H_DATA: #{h_data.to_json}"
                load_annot(run, h_data, relative_filepath, h_data_types)
                ...
                if  h_metadata_by_name.keys.size > 0
                  h_metadata_by_name.each_key do |meta_name|
                    ...
                    load_annot(run, metadata, relative_filepath, h_data_types)
                  end
                end
              elsif h_output_json['metadata'] and metadata = h_metadata_by_name[dataset_name]
          ...
          if h_output_json['output_files'] and h_output_json['output_files'][k2] and h_output_json['output_files'][k2]["types"]
            h_output_files[k][k2]["types"] |= h_output_json['output_files'][k2]["types"]
          end
        end
      end
