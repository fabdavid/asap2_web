  ...
  #        puts "Updated!"
    ...
    def format_author e
      ...
      if e['family']
        ...
        initials = ''
        if e['given'].match(/^[A-Z]+$/)
          initials = e['given']
        elsif e['given'].match(/\./)
          initials = e['given'].gsub(/\./, '')
        else
          initials = e['given'].gsub(/[a-z]/, '') #split(/+/).map{|g| g.first}.join("")                                                                                                                                                        
        end
        h_author = {
          'initials' => initials, #(e['given'].gsub(/\./, '').match(//)) ,                                                                                                                                                                     
          'lname' => fname, #(e['family'].upcase == e['family']) ? e['family'].downcase.capitalize : e['family'],                                                                                                                              
          'aff_ids' => []
        }
      end
      return h_author
