...
function heatmap(selector, data, options) {
  ...
  colormapBounds.top=110;
  colormapBounds.left=110;
  var colDendBounds = gridSizer.getCellBounds(1, 0);
//  alert(colDendBounds.top);
//  colDendBounds.top=20;//colDendBounds.height-100;
  colDendBounds.left= 110;
  colDendBounds.height=100;
  var rowDendBounds = gridSizer.getCellBounds(0, 1);
//  rowDendBounds.left=0// rowDendBounds.width-100;
  rowDendBounds.top=110;
  rowDendBounds.width=100;
  
  var yaxisBounds = gridSizer.getCellBounds(2, 1);
yaxisBounds.top=110;
yaxisBounds.left=colormapBounds.width + 110;

  var xaxisBounds = gridSizer.getCellBounds(1, 2);
xaxisBounds.top=colormapBounds.height + 110;
xaxisBounds.left=110; 
  ...
  var row = !data.rows ? null : dendrogram(el.select('svg.rowDend'), data.rows, false, 100, //rowDendBounds.width, 
					   rowDendBounds.height, opts.axis_padding);
  var col = !data.cols ? null : dendrogram(el.select('svg.colDend'), data.cols, true, colDendBounds.width, 100,//colDendBounds.height,
					   opts.axis_padding);
  var colormap = colormap(el.select('svg.colormap'), data.matrix, data.image, colormapBounds.width, colormapBounds.height);
...
function loadImage(uri, callback){
 var img = new Image();
  ...
  function colormap(svg, data, uri, width, height) {
    // Check for no data
    if (data.length === 0)
      return function() {};
...
alert(merged[0].color + " " + merged[1].color);

      // Done with the canvas, remove it from the page so it can be gc'd.                                                                                                                        
//      document.body.removeChild(imgDataCanvas);

      callback(imgData, merged, w, h);
    };
    img.src = uri;

*/

  loadImage(uri, function(imgData, w, h) {
...
//      alert(imgData);
   ...
   // rect.call(tip);
    ...
    brushG.select("rect.background")
	    ...
	    $('#point_details').html("x: #" + col + " " + data.cols[col] + "; y: #" + row + " " + data.rows[row] + "<br/>Value:" + label);
	    $('#point_details').css({left: e.pageX - $(document).scrollLeft(), top: e.pageY - $(document).scrollTop()});
	    $('#point_details').removeClass("hidden");
          controller.datapoint_hover({col:col, row:row, label:label});
        })
        .on("mouseleave", function() {
        //  tip.hide().style("display", "none");
	     $('#point_details').addClass("hidden")
