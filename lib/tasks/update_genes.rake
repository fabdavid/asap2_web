...
task update_genes: :environment do
  ...
  def extract_genes start, db_type, release_num
    ...
    ensembl_tables = ['seq_region', 'gene', 'external_db', 'xref', 'object_xref', 'transcript', 'external_synonym', 'exon_transcript', 'exon'] 
  ...
  [:bacteria, :fungi, :metazoa, :plants, :protists].each do |db_type|
    ...
    if initial_release <= final_release
      ...
      puts "#{db_type.to_s.capitalize} is ALREADY up to date! (#{initial_release} - #{final_release})"
    end
  end
  #end
