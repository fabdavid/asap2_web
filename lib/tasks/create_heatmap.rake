desc '####################### Create heatmap'
task :create_heatmap, [:pdr_id] => [:environment] do |t, args|
  puts 'Executing...'
  ...
  ### empty directory                                                                                                                                                     # puts "empty directory #{visualization_dir}..."  
...
#puts h_genes.to_json
  ...
  if h_output[:displayed_error]
    ...
    cmd = ["ulimit -s 16384 && Rscript --vanilla --max-ppsize=500000 lib/heatmap.R", input_file, visualization_dir, h_parameters['dist_method'], h_parameters['clust_method']].join(" ")   
